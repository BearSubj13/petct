...
def train_epoch(model, lungs_data_loader, encoder_optimizer, discriminator_optimizer, decoder_optimizer,
 current_lod, blend_factor=1, freeze_previous_layers=None, lambda_reconstruct=0, p_augment=0.5):
    ...
    result = {'d':mean_loss_d, 'g':mean_loss_g, 'lae':mean_loss_lae, \
              ...
              'lae_rec':mean_loss_latent_reconstruction, 'mse_rec':mean_loss_image_reconstruction}
    return result
...
if __name__ == "__main__":
    ...
    resolution_power = 2
    load_model_path = None#'weights/model64_5layers.pth'
    ...
    ], lr=0.002, weight_decay=weigt_decay)
    ...
    for epoch in range(400):
        ...
        format_output = "{0:3d}  d:{1:2.5f}, g:{2:2.5f}, lae:{3:2.5f}    lae rec:{4:2.5f}, mse:{5:2.5f}, g rec:{6:2.5} , d rec:{7:2.5}". \
            format(epoch, loss['d'], loss['g'], loss['lae'], loss['lae_rec'], \
                 loss['mse_rec'], loss['g_rec'], loss['d_rec'])         
        print(format_output)

        if loss['mse_rec'] < 0.01 and epoch > 10:
